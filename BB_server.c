...
int process_request (int, int, double [2], double [2] [obs_num], double [2] [targ_num], char [240], char [240], int *, int *);
...
int main (int argc, char* argv[] ) {
    ...
    if (process_request (2, fd_out [2], drone_pos, obstacle_pos, targ_pos, request_string, answer_string, obs_changed, &obs_placed) < 0) {
    ...
    while (1) {
        ...
        switch (write_req) {
            ...
            case 1: //only one request: no need to randomly choose but need to identify requester (its id) for proper action (fd- and data transmission-wise)
                ...
                if (process_request (req_idx, fd_out [req_idx], drone_pos, obstacle_pos, targ_pos, request_string, answer_string, obs_changed, &obs_placed) < 0) {
...
int process_request (int req_idx, int request_fd_o, double drone_pos [2], double obs_pos [2] [obs_num], double targ_pos [2] [targ_num], char pipe_msg [240], char out_msg [240], int * changed_obstacles, int * obs_placed) {
    char tmp_msg [240];
    switch (req_idx) {
